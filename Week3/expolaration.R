...
#Load files
load("./Data/data_20171120_20171124.RData")
...
calculate_volatitlity <- function() {
  assets = names(data_format1)
  N = nrow(data_format1[['AU.SHF']])
  for (asset in assets) {
    asset_data = data_format1[[asset]]
    vol=volatility(asset_data$close,n=25,N=N,calc='close')
    plot(vol,type='l',main=asset)
  }
}
...
fit_arima_model <- function(return = "high") {
  ...
  for (asset in assets){
    ...
    #For plotting graphs
    acf_label_name = paste("ACF", asset, sep=" ")
    pacf_label_name = paste("PACF", asset, sep=" ")
    arima_label_name = paste("ARIMA Auto,Low,High", asset, sep=" ")
    log_low_high_label_name = paste("Log,Low,High", asset, sep=" ")
    
    acf.asset_data=acf(log.asset_data,main=acf_label_name,lag.max=100,ylim=c(-0.5,1))
    pacf.asset_data=pacf(log.asset_data,main=pacf_label_name,lag.max=100,ylim=c(-0.5,1))
    ...
    plot(forecast_arima, main=arima_label_name)
    fit212=fitted.values(autoarima)
    low=fit212-1.96*sqrt(ht.arch08)
    high=fit212+1.96*sqrt(ht.arch08)
    plot(log.asset_data,type='l',main=log_low_high_label_name)
    lines(low,col='red')
    lines(high,col='blue')
